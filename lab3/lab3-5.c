...
void display(void)
{
    ...
    glUniform1i(glGetUniformLocation(program, "use_tex"), 2);
    glUniform1i(glGetUniformLocation(program, "texUnit"), 2); // dirt
    glUniform1i(glGetUniformLocation(program, "secondTexUnit"), 0); // grass

    glEnable(GL_CULL_FACE);
    glCullFace(GL_FRONT);
    draw(bunnyModel, Mult(T(0,5,-30), S(10,10,10)));
    glCullFace(GL_BACK);
    draw(bunnyModel, Mult(T(0,5,-30), S(10,10,10)));
    glDisable(GL_CULL_FACE);

	glUniform1i(glGetUniformLocation(program, "use_tex"), 0);
	draw(bunnyModel, Mult(Mult(T(55,2.5,14), S(5,5,5)),Ry(a)));
